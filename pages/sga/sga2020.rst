...
``RADIUS_SB[22,22.5,23,23.5,24,24.5,25,25.5,26]``          float32      arcsec                                      Semi-major axis length at the  22, 22.5, 23, 23.5, 24, 24.5, 25, 25.5, and 26 :math:`26\ \mathrm{mag}\ \mathrm{arcsec}^{-2}` isophote in the *r*-band (-1 if not measured); taken directly from the `Ellipse Fits`_ table.

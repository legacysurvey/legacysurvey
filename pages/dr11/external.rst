...
Several external catalogs are used for astrometric and photometric calibration of DR11. These catalogs are currently available in the indicated directories at NERSC.
...
External catalogs are used for masking regions near foreground sources in DR11
...
"BRIGHT" stars
--------------
| **/global/cfs/cdirs/cosmo/data/legacysurvey/dr11/masking/gaia-mask-dr11.fits.gz**
| https://portal.nersc.gov/cfs/cosmo/data/legacysurvey/dr11/masking/gaia-mask-dr11.fits.gz
|     Bright stars are defined from a starting sample of all sources in the Tycho-2 catalog that have ``MAG_VT`` < 13.  The ``BRIGHT`` bit is set for all such Tycho-2 stars. In addition, Gaia sources with ``phot_g_mean_mag`` < 13 have the ``BRIGHT`` bit set, provided they do not already match a Tycho-2 source. Gaia and Tycho-2 sources are matched after accounting for proper motion. In the **gaia-mask-dr11-near.fits.gz** file, bright stars have the ``isbright`` column set to ``True``. In the legacypipe code, the radius of the ``BRIGHT`` star mask is set by pixels within `half of`_ the locus of a hardcoded `radius-magnitude relation`_ that is set for all Tycho-2 stars and for Gaia stars `to G < 13`_. But, note that the specific radii in the **gaia-mask-dr11-near.fits.gz** do not include the factor-of-two correction (i.e. they are the appropriate radii for the ``MEDIUM`` masks, as described below).

"MEDIUM-bright" stars
---------------------
| **/global/cfs/cdirs/cosmo/data/legacysurvey/dr11/masking/gaia-mask-dr11.fits.gz**
| https://portal.nersc.gov/cfs/cosmo/data/legacysurvey/dr11/masking/gaia-mask-dr11.fits.gz
|     Medium-bright stars are also defined starting with all sources in the Tycho-2 catalog cut to ``MAG_VT`` < 13.  All such Tycho-2 stars have the ``MEDIUM`` bit set. In addition, Gaia sources with ``phot_g_mean_mag`` < 16 (`G < 16`_) have the ``MEDIUM`` bit set, provided they do not already match a Tycho-2 source (where the match accounts for proper motion). Note that this means that all ``BRIGHT`` stars also have the ``MEDIUM`` bit set. The specific `radius-magnitude relation`_ used is the same one that is adopted for ``BRIGHT`` masks. In the **gaia-mask-dr11-near.fits.gz** file, medium-bright stars have the ``ismedium`` column set to ``True``. Note that, in the legacypipe code, the radius of a ``BRIGHT`` mask is half that for a ``MEDIUM`` mask for a star of the same magnitude. The radii in the **gaia-mask-dr11-near.fits.gz** file *correspond to the radii for the MEDIUM masks* (which is *twice* the appropriate radius for a ``BRIGHT`` mask).

.. _`radius-magnitude relation`: https://github.com/legacysurvey/legacypipe/blob/DR11.1.0/py/legacypipe/reference.py#L452-L457
.. _`to G < 13`: https://github.com/legacysurvey/legacypipe/blob/DR11.1.0/py/legacypipe/reference.py#L448
...
| **/global/cfs/cdirs/cosmo/data/legacysurvey/dr11/masking/NGC-star-clusters.fits**
| https://portal.nersc.gov/cfs/cosmo/data/legacysurvey/dr11/masking/NGC-star-clusters.fits
|     In DR11, we treat globular clusters and planetary nebulae by turning off source detection (although we do perform forced photometry of Gaia sources) and using the ``CLUSTER`` bit to identify the regions of the sky occupied by these systems. We construct our catalog using the following procedure:
...
3. Finally, we add two Local Group galaxies to this catalog, `Fornax`_ and
   ...
   occupied by these galaxies DR11 only contains photometry for Gaia point
   sources.
...
Large Galaxies
--------------
| **/global/cfs/cdirs/cosmo/data/legacysurvey/dr11/masking/SGA2025-ellipse-v1.6-dr11-north.kd.fits** and
...
.. _`NGC-star-clusters-radii.csv`: https://github.com/legacysurvey/legacypipe/blob/DR11.1.0/py/legacypipe/data/NGC-star-clusters-radii.csv
.. _`star-clusters-supplemental.csv`: https://github.com/legacysurvey/legacypipe/blob/DR11.1.0/py/legacypipe/data/star-clusters-supplemental.csv

...
We note that the `NASA Extragalactic Database (NED)`_ proved invaluable for
these cross-checks.
...
3. With the images and individual masks for each galaxy in hand, we can now
   ...
   *nclip=3*, *sclip=3*, and *integrmode=median*, i.e., two sigma-clipping
   ...
   empirical model (taken from `Observational Astronomy by Birney, Gonzalez, &
   Oesper`_):
...
`SGA acknowledgment`_, which recognizes the funding and invaluable external
resources and datasets which made this project possible.
